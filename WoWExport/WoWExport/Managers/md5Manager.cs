...
namespace Managers
{
    class md5Manager
    {
        ...
        {
            using (StreamReader reader = new StreamReader(Managers.ArchiveManager.ReadThisFile(@"textures\minimap\md5translate.trs")))
            {
                string line;
        ...
        public static void ExportAllMinimaps(string to)
        {
            ...
            for (int i = 0; i < lines.Count(); i++)
            {
                ...
                try
                {
                    minireader.LoadBLP(Managers.ArchiveManager.ReadThisFile(@"textures\minimap\" + hash));
        ...
        public static String TranslateThisMap(string filename)
        {
            ...
            if (index != -1)
            {
                //string file = lines[index].Substring(0, lines[index].IndexOf("\t")); //path + filenames
